...
public class Schedule {
    ...
    public boolean breakAtTime(String dayOfWeek, String time){
        SortedSet<Section> sections = getSectionsForDayOfWeek(dayOfWeek);
        String breakStartTime = time + ":00";
        String breakEndTime = String.valueOf(Integer.parseInt(time) + 1) + ":00";
        Log.d("Schedule", "looking for a break between " + breakStartTime + " and " + breakEndTime);
        if (sections.isEmpty()) {
            Log.d("Schedule", "sections empty");
            return true;
        }
        Section lastSection = null;
        for (Section section : sections) {
            if (section.getCourseTime().getStartTime().compareTo(breakEndTime) >= 0) { //if section starts after the break
                Log.d("Schedule", section.getCourse().getCode() + section.getCourse().getNumber() + section.getName() + "starts after break");
                if (lastSection == null || lastSection.getCourseTime().getEndTime().compareTo(breakStartTime) <= 0){ //if it's the first section of the day, or the last section ended before the break
                    if (lastSection != null) Log.d("Schedule", lastSection.getCourse().getCode() + lastSection.getCourse().getNumber() + lastSection.getName() + "ends before the break");
                    Log.d("Schedule", "returning true");
                    return true;
                }else {
                    Log.d("Schedule", lastSection.getCourse().getCode() + lastSection.getCourse().getNumber() + lastSection.getName() + "ends after the break starts");
                    return false;
                }
            }else {
                Log.d("Schedule", section.getCourse().getCode() + section.getCourse().getNumber() + section.getName() + "starts before break");
                lastSection = section;
            }
        }
        Log.d("Schedule", "Last course ends before the break, returning true");
        return true;
    }
